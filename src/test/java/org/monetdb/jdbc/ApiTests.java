...
public class ApiTests {
	...
	@Test
	public void testBogusAutoGeneratedKeys() throws SQLException {
		// The name indicates the test is about generated keys but
		// I see no calls to Statement#getGeneratedKeys() here.
		// Don't know what's going on here.
		startScratchTransaction();

		assertEquals(0, stmt.executeUpdate("CREATE TABLE bogus_gen_keys (id SERIAL, x VARCHAR(12))"));
		assertEquals(0, stmt.executeUpdate("UPDATE bogus_gen_keys SET x = 'bla' WHERE id = 12;"));
		assertEquals(1, stmt.executeUpdate("INSERT INTO bogus_gen_keys(x) VALUES ('boe')"));
		try (ResultSet rs = stmt.executeQuery("SELECT id, x FROM bogus_gen_keys")) {
			assertTrue(rs.next());
			assertEquals("1", rs.getString(1)); // why getString?
			assertEquals("boe", rs.getString(2));
			assertFalse(rs.next());
		}
